...
db_intro_new_content = """# Modul: Database & ORM - Mengenal Gudang Data
...
sql_queries_new_content = """# Modul: SQL Queries - Berbicara dengan Penjaga Data
...
def indent_content(text, spaces=10):
    lines = text.strip().split('\n')
    indented_lines = [(' ' * spaces + line) if line.strip() else '' for line in lines]
    return '|\n' + '\n'.join(indented_lines)

# Replace db-intro content
# Pattern looks for id: db-intro, title, and then the content until quiz: or another - id:
db_intro_pattern = r'(- id: db-intro\s+title: [^\n]+\s+content: )("[^"]*"|\|.*?\n(?=\s+quiz:|\s+- id:))'
content = re.sub(db_intro_pattern, r'\1' + indent_content(db_intro_new_content), content, flags=re.DOTALL)

# Replace sql-queries content
sql_queries_pattern = r'(- id: sql-queries\s+title: [^\n]+\s+content: )("[^"]*"|\|.*?\n(?=\s+quiz:|\s+- id:))'
content = re.sub(sql_queries_pattern, r'\1' + indent_content(sql_queries_new_content), content, flags=re.DOTALL)

with open(file_path, 'w', encoding='utf-8') as f:
    f.write(content)

print("Success: Lessons updated via regex.")
